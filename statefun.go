...
const (
	...
	_CONTROLLER_RESULT_TYPE  = "ui_controller_result"
)
...
func RegisterAllFunctionTypes(runtime *statefun.Runtime, opts ...UIOpt) {
	...
	statefun.NewFunctionType(runtime, h.cfg.IngressTopic, h.ingress, *statefun.NewFunctionTypeConfig().SetMaxIdHandlers(-1))
	statefun.NewFunctionType(runtime, sessionFunction, h.session, *statefun.NewFunctionTypeConfig().SetMaxIdHandlers(-1))
	statefun.NewFunctionType(runtime, clientEgressFunction, h.egress, *statefun.NewFunctionTypeConfig().SetMaxIdHandlers(-1))
	...
	statefun.NewFunctionType(runtime, controllerConstruct, h.controllerConstruct, *statefun.NewFunctionTypeConfig().SetMaxIdHandlers(-1))
	statefun.NewFunctionType(runtime, controllerResultCompare, h.controllerResultCompare, *statefun.NewFunctionTypeConfig().SetMaxIdHandlers(-1))
